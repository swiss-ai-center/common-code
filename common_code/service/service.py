...
class ServiceService:
    ...
    async def graceful_shutdown(self, my_service: Service, engine_url: str):
        ...
        try:
            ...
            service_json = jsonable_encoder({"status": ServiceStatus.SLEEPING})
    ...
    async def heartbeat(self, engine_url: str, my_service: Service):
        ...
        try:
            service_json = jsonable_encoder(my_service)
            res = await self.http_client.post(f"{engine_url}/services/{my_service.slug}/ping", json=service_json)
